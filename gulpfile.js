...
// don't do a sourcemap for wp receipe maker
gulp.task('bean', function () {
	return gulp
		.src('wprm-templates/recipe/bean/bean.scss')
		.pipe(sassGlob())
		.pipe(sass().on('error', sass.logError))
		.pipe(sourcemaps.write())
		.pipe(cleanCSS({ compatibility: 'ie8' }))
		.pipe(gulp.dest('wprm-templates/recipe/bean/'));
});
...
gulp.task('dev', function () {
	...
	gulp.watch('wprm-templates/recipe/**/*.scss', gulp.series(['bean']));
});
